...
async function fetchMemes(memeUrl) {
  try {
    ...
    for (let i = 0; i < results.length; i++) {
      memeContainer.innerHTML += `<div class="meme_wrapper"><img src="${results[i].images[0].src}" class="meme" tabindex="0" alt="${results[i].images[0].alt}"></div>
    ...
    console.log(error);
    memeContainer.innerHTML = displayError(
      "An error occured when calling API, please refresh or try again later"
    );
  }
}
